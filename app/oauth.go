...
func (a *App) TFAuthorizeOAuthUser(w http.ResponseWriter, r *http.Request, service, signedAttempt string) (io.ReadCloser, string, map[string]string, *model.AppError) {

	cookie, cookieErr := r.Cookie(COOKIE_STATE)
	if cookieErr != nil {
		return nil, "", nil, model.NewAppError("TFAuthorizeOAuthUser", "Failed to read cookie, make sure your browser allows cookies for this website", nil, "Failed to read cookie, make sure your browser allows cookies for this website", http.StatusNotImplemented)
	}
	state := cookie.Value

	sso := a.Config().GetSSOService(service)
	if sso == nil || !*sso.Enable {
		return nil, "", nil, model.NewAppError("TFAuthorizeOAuthUser", "api.user.authorize_oauth_user.unsupported.app_error", nil, "service="+service, http.StatusNotImplemented)
	}
	p := url.Values{}
	...
	if requestErr != nil {
		return nil, "", nil, model.NewAppError("TFAuthorizeOAuthUser", "could not create post request to Oauth proxy server", nil, requestErr.Error(), http.StatusNotImplemented)
	}
	...
	if err != nil {
		return nil, "", nil, model.NewAppError("TFAuthorizeOAuthUser", "Couldn't send verify request to Oauth proxy server", nil, err.Error(), http.StatusNotImplemented)
	}
	if resp.StatusCode != http.StatusOK {
		return nil, "", nil, model.NewAppError("TFAuthorizeOAuthUser", "Verification request to Oauth proxy server returns bad status", nil, fmt.Sprint("Verification request to Oauth proxy server returns status: %d", resp.StatusCode), http.StatusNotImplemented)
	}

	b, strErr := base32.StdEncoding.DecodeString(state)
	if strErr != nil {
		return nil, "", nil, model.NewAppError("AuthorizeOAuthUser", "Failed to decode state object", nil, strErr.Error(), http.StatusBadRequest)
	}
